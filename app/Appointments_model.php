...
class Appointments_Model extends CI_Model {
    ...
    public function add($appointment)
    {
         ...
         public function select_user($id)
        {
         ...
         public function update_login_user($dataupdate,$id)
        {
            $this->db->where('id', $id);
            $this->db->update('ea_users', $dataupdate); 
        }
    /**
    ...
    public function forget_password($email, $passwor)
    {
       $this->db->where('email', $email);
     $this->db->update('ea_users', $passwor);
    }

        public function accoun_activate($email, $login)
    {
       $this->db->where('email', $email);
     $this->db->update('ea_users', $login);
    }
   ...
   public function filename_exists($email)
{
    ...
    return $result;
     ...
     public function appointments_quarr($id,$data)
    { 
          $this->db->where('id', $id);
        $this->db->update('ea_appointments', $data);
    }
 public function get_last_record(){
   ...
   //      foreach ($data['datecheck'] as $key => $value) {
   //       // $newValue = substr($value, 0, -3);
   //      $date = str_replace('/', '-', $value );
   //      $newDate = date("y-m-d h:i:s", strtotime($date));
   //  $cenvertedTime = date('y-m-d h:i:s',strtotime('+1 hour',strtotime($newDate)));

   //           $dataArr = array(
   //          'book_datetime'=>$current,
   //          'start_datetime'=>$newDate,
   //          'end_datetime'=>$cenvertedTime,
   //          'id_users_provider'=>$data['service_provide'],
   //          'id_users_customer'=>$id,
   //          'id_services'=>$data['service_id'],
   //          'is_unavailable'=>'0',
   //          'notes' => $data['comment'],
   //          'hash' => $hash
   //        );
   //           $this->db->insert('ea_appointments',$dataArr);
